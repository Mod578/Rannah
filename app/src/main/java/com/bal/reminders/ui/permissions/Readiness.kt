...
enum class ReadinessIssue(val titleRes: Int, val bodyRes: Int, val blocking: Boolean) {
    ...
    /** Reminders still arrive, but late, which for «بصمة الدوام» is failure. */
    EXACT_ALARMS(
    ...
    FOLLOW_UP_CHANNEL(
        R.string.readiness_followup_channel_title,
        R.string.readiness_followup_channel_body,
        blocking = false,
    ),
...
fun PermissionsStatus.issues(): List<ReadinessIssue> = buildList {
    ...
    if (followUpChannelBlocked) add(ReadinessIssue.FOLLOW_UP_CHANNEL)
