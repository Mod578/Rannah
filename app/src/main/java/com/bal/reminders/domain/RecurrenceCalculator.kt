...
object RecurrenceCalculator {
    ...
    /**
     * Whether the announced Hijri (month, day) exists as stated this coming
     * cycle, or will clamp (day 30 in a 29-day month). Used by the editor to
     * explain the limitation instead of silently changing meaning.
     */
    fun hijriDayClamps(month: Int, day: Int, around: LocalDate, adjustmentDays: Int = 0): Boolean {
        if (day < 30) return false
        val year = HijriDates.fromGregorian(around, adjustmentDays)
            ?.get(ChronoField.YEAR) ?: return false
        val length = HijriDates.monthLength(year, month) ?: return false
        return day > length
    }
}
