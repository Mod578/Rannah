...
import com.bal.reminders.data.AppSettings
...
/**
 * ودجت الشاشة الرئيسية: تاريخ اليوم (هجري/ميلادي حسب تفضيل المستخدم)
 * والرنّة القادمة. تتحدث كل ٣٠ دقيقة ومع كل تغيير في التذكيرات.
 */
@AndroidEntryPoint
class RannaWidgetProvider : AppWidgetProvider() {
    ...
    override fun onUpdate(
        ...
        CoroutineScope(SupervisorJob() + Dispatchers.IO).launch {
            try {
                ...
                val views = buildViews(context, settings, next)
    ...
    private fun buildViews(
        ...
        val (primaryDate, secondaryDate) = BalFormats.dateLines(
            LocalDate.now(),
