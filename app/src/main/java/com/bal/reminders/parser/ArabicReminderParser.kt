 ...
 * remaining original words become the title.
 *
 * Supported grammar (examples):
 * - «ذكرني بكرة الساعة 8 بالاجتماع»            one-time, tomorrow
 * - «ذكرني كل يوم الساعة 9 ببصمة الدوام»       daily
...
class ArabicReminderParser @Inject constructor() : ReminderParser {
    ...
    override fun parse(input: String, now: ZonedDateTime): ParseResult {
        ...
        val hijriMonthlyDay = extractHijriMonthly(text)
        val monthlyDay = if (hijriMonthlyDay == null) extractMonthly(text, now) else null
        ...
        val recurring =
            hijriMonthlyDay != null || monthlyDay != null || weeklyDays != null || daily || yearly
        ...
        val schedule: Schedule? = when {
            ...
            hijriMonthlyDay != null -> rawTime?.let { Schedule.HijriMonthly(hijriMonthlyDay, resolveTime(it)) }
        ...
        return when {
            ...
            schedule != null ->
                ...
                val partial: Schedule? = when {
                    ...
                    hijriMonthlyDay != null -> Schedule.HijriMonthly(hijriMonthlyDay, placeholder)
                    weeklyDays != null -> Schedule.Weekly(weeklyDays, placeholder)
                    daily -> Schedule.Daily(placeholder)
                    dateRef is DateRef.HijriDayMonth ->
                        Schedule.OnceHijri(
                            hijriYearFor(dateRef, now, placeholder),
                            dateRef.month,
                            dateRef.day,
                            placeholder,
                        )
    ...
    private fun extractHijriMonthly(text: ConsumableText): Int? =
        text.consumeFirstIf(HIJRI_MONTHLY) { it.groupValues[1].toInt() in 1..30 }
    ...
    private fun extractDateRef(text: ConsumableText): DateRef? {
        ...
        text.consumeFirstIf(HIJRI_DAY_MONTH) {
            val day = if (it.groupValues[1].isNotEmpty()) it.groupValues[1].toInt() else 1
            day in 1..30
        }?.let {
            val day = if (it.groupValues[1].isNotEmpty()) it.groupValues[1].toInt() else 1
            return DateRef.HijriDayMonth(day, HIJRI_MONTHS.getValue(it.groupValues[2]))
        }
    ...
    private fun onceForDate(ref: DateRef, raw: RawTime, now: ZonedDateTime): Schedule {
        ...
        return when (ref) {
            ...
            // The Hijri calendar is preserved as the reminder's real semantics.
            is DateRef.HijriDayMonth ->
                Schedule.OnceHijri(hijriYearFor(ref, now, time), ref.month, ref.day, time)
        }
    }

    /**
     * «١٥ شعبان كل سنه» → Hijri yearly; «25/1 كل سنه» → Gregorian yearly;
     * «يوم 25 كل سنه» assumes the current month.
     */
    private fun yearlySchedule(ref: DateRef, time: LocalTime, now: ZonedDateTime): Schedule? =
        when (ref) {
            is DateRef.HijriDayMonth -> Schedule.HijriYearly(ref.month, ref.day, time)
    ...
    /** The Hijri year of the next calculated occurrence of (month, day). */
    private fun hijriYearFor(ref: DateRef.HijriDayMonth, now: ZonedDateTime, time: LocalTime): Int {
        var year = HijrahDate.from(now.toLocalDate()).get(ChronoField.YEAR)
        repeat(3) {
            val civil = HijriDates.toGregorian(year, ref.month, ref.day)
            if (civil != null && civil.atTime(time).atZone(now.zone).isAfter(now)) return year
            year++
        }
        return year
    }
        ...
        return when (ref) {
            ...
            is DateRef.HijriDayMonth ->
                HijriDates.toGregorian(
                    hijriYearFor(ref, today.atStartOfDay(java.time.ZoneOffset.UTC), LocalTime.MAX),
                    ref.month,
                    ref.day,
                ) ?: today
        }
    ...
    private sealed interface DateRef {
        ...
        data class HijriDayMonth(val day: Int, val month: Int) : DateRef
    }
    ...
    private companion object {
        ...
        val HIJRI_MONTHLY = w("(?:يوم )?(\\d{1,2}) من كل شهر هجري")

        val AFTER_TOMORROW = w("بعد بكره|بعد بكرا|بعد غد|بعد غدا")
        val TOMORROW = w("بكره|بكرا|غدا|غد|باكر|باجر")
        val TODAY = w("اليوم|النهارده")

        const val HIJRI_MONTH_ALT =
            "محرم|صفر|ربيع الاول|ربيع الثاني|جمادى الاولى|جمادى الاول|جمادى الاخره|رجب|شعبان|رمضان|شوال|ذو القعده|ذي القعده|ذو الحجه|ذي الحجه"
        val HIJRI_DAY_MONTH = w("(?:(?:اول|1)|(\\d{1,2})) ($HIJRI_MONTH_ALT)")
        val HIJRI_MONTHS = mapOf(
            "محرم" to 1, "صفر" to 2, "ربيع الاول" to 3, "ربيع الثاني" to 4,
            "جمادى الاولى" to 5, "جمادى الاول" to 5, "جمادى الاخره" to 6,
            "رجب" to 7, "شعبان" to 8, "رمضان" to 9, "شوال" to 10,
            "ذو القعده" to 11, "ذي القعده" to 11, "ذو الحجه" to 12, "ذي الحجه" to 12,
        )
