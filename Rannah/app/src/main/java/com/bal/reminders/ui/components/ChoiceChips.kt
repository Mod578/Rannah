...
fun <T> ChoiceChips(
    ...
    FlowRow(
        ...
        // Chips that wrap need vertical air too. Without it the second row sits
        // flush against the first and the group reads as one cramped block.
...
/** The same chip group, but any number can be selected at once (e.g. weekdays). */
@OptIn(ExperimentalLayoutApi::class)
@Composable
fun <T> ChoiceChipsMulti(
    options: List<Pair<T, String>>,
    selected: Set<T>,
    onToggle: (T) -> Unit,
    modifier: Modifier = Modifier,
