...
import com.bal.reminders.data.db.DeletedReminderEntities
...
class ReminderRepositoryImpl @Inject constructor(
    ...
    override suspend fun deleteWithRecords(id: Long): DeletedReminder? =
        dao.deleteWithRecords(id)?.let { removed ->
            DeletedReminder(
                reminder = removed.reminder.toDomain(),
                records = removed.records.map { it.toDomain() },
            )
        }

    override suspend fun restore(deleted: DeletedReminder) = dao.restore(
        DeletedReminderEntities(
            reminder = deleted.reminder.toEntity(),
            records = deleted.records.map { it.toEntity() },
        ),
    )
    ...
    override suspend fun setSnooze(id: Long, until: Instant?, occurrence: Instant?) =
        dao.setSnooze(id, until?.toEpochMilli(), occurrence?.toEpochMilli())
    ...
    override suspend fun pruneCompletedOnceBefore(before: Instant): Int =
        dao.pruneCompletedOnceBefore(before.toEpochMilli())

    override suspend fun pruneRecordsBefore(before: Instant) =
        dao.pruneRecordsBefore(before.toEpochMilli())
}
